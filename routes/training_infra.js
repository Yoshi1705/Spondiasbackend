...
const table_name = 'training_infra';
...
router.put('/:id', (req, res) => {
    pool.getConnection((err, connection) => {
        ...
        connection.query(`UPDATE ${table_name} SET image=?, status= ?, updated_at= ?  WHERE id= ?`, [req.body.image, req.body.status, date, id], function (error, results, fields) {
            if (error) throw error;
            res.send(results);
        });
