...
router.put('/:id', upload.single('image'), (req, res) => {
    pool.getConnection((err, connection) => {
        ...
        connection.query(`UPDATE ${table_name} SET menu_title= ?, menu_description=?, image=?, status= ?, updated_at= ?  WHERE id= ?`, [req.body.menu_title, req.body.menu_description, req.file.filename, req.body.status, date, id], function (error, results, fields) {
            if (error) throw error;
            res.send(results);
        });
