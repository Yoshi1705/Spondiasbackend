...
const table_name = 'slider_details';
...
router.put('/:id', upload.single('image'), (req, res) => {
    pool.getConnection((err, connection) => {
        ...
        connection.query(`UPDATE ${table_name} SET title=?, description=?, image=?, status=?, updated_at= ?  WHERE id= ?`, [req.body.title, req.body.description, req.file.filename, req.body.status, date, id], function (error, results, fields) {
            if (error) throw error;
            res.send(results);
        });
