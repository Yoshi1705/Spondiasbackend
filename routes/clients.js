...
router.put('/:id', upload.single('logo'), (req, res) => {
    pool.getConnection((err, connection) => {
        ...
        connection.query(`UPDATE ${table_name} SET logo= ?, link= ?, status= ?, updated_at= ?  WHERE id= ?`, [req.file.filename, req.body.link, req.body.status, date, id], function (error, results, fields) {
            if (error) throw error;
            res.send(results);
        });
